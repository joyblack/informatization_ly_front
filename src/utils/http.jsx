...
let addTokenToHeaders = (json) => {
  ...
  let Authorization
  if (sessionStorage.getItem(baseConstant.Authorization)) {
    Authorization = sessionStorage.getItem(baseConstant.Authorization)

  }
  return {
    'Accept': 'application/json',
    'Content-Type': json ? 'application/json' : 'application/x-www-form-urlencoded',
    'Authorization-GM': Authorization,
  }
}
/**
 * 添加请求头
...
let disposeResponse = (response) => {
  if (response.headers.get(baseConstant.ManagersAuthorization)) {
    sessionStorage.Authorization = response.headers.get(baseConstant.ManagersAuthorization)
  }
  return response
}
/**
...
let disposeError = (res) => {
  if (res.success) {
    ...
    if (baseConstant.toLoginMsg.indexOf(res.message) > -1) {
...
http.post = (url, params, json, np) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(url, {
      ...
      body: json ? JSON.stringify(params) : querystring.stringify(params),
      mode: 'cors'
    }).then(disposeResponse).then(parseJSON).then(disposeError)
      .then(result => {
        if (np) {
